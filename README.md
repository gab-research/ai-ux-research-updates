...
2. **That’s it**
   - GitHub Actions will run **once per day** (at 12:00 UTC) and:
     ...
     - Rewrite `updates.json` with the newest articles and their real source URLs.
...
- **scripts/update-from-feeds.js** — Fetches those feeds, filters by keywords, and writes `updates.json`.
