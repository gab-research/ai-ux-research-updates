...
async function main() {
  ...
  const maxUpdates = Math.min(Number(sources.maxUpdates) || 20, 50);
  ...
  const updates = allItems.slice(0, maxUpdates);

  // If no items found (e.g. all feeds failed), keep existing updates so the site isn't wiped
  const existingUpdates = existing && Array.isArray(existing.updates) ? existing.updates : [];
  const finalUpdates = updates.length > 0 ? updates : existingUpdates;
